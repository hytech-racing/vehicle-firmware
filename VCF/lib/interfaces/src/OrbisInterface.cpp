...
{
   _last_reading.status = SteeringEncoderStatus_e::ERROR;
...
bool OrbisInterface::perform_self_calibration()
{
    ...
    */
    ...
    {
        _orbis_errors.calibration_timeout   = timeout_error;
        _orbis_errors.calibration_parameter = parameter_error;
        return false;
    }
...
void OrbisInterface::sample()
{
    ...
    _last_reading.errors = EncoderErrorFlags_s{};  // reset to all false
    _orbis_errors        = OrbisErrorFlags_s{};    // reset to all false
    ...
    while (_serial->available() < 4)
    {
        ...
        {
            _last_reading.errors.noData = true;
            _last_reading.status = SteeringEncoderStatus_e::ERROR;
            return;
        }
    }
    ...
    _last_reading.rawValue = raw_position;
...
void OrbisInterface::_decode_errors(uint8_t general, uint8_t detailed)
{
    ...
    _last_reading.errors.operatingLimit   = !(general & orbis_bitmasks::GENERAL_WARNING_BITMASK);

    // Detailed bits are error high (1)
    _orbis_errors.counter_error      = detailed & orbis_bitmasks::DETAILED_COUNTER_ERROR_BITMASK;
    _orbis_errors.speed_high         = detailed & orbis_bitmasks::DETAILED_SPEED_HIGH_BITMASK;
    _orbis_errors.temp_out_of_range  = detailed & orbis_bitmasks::DETAILED_TEMP_RANGE_BITMASK;
    _orbis_errors.dist_far           = detailed & orbis_bitmasks::DETAILED_DIST_FAR_BITMASK;
    _orbis_errors.dist_near          = detailed & orbis_bitmasks::DETAILED_DIST_NEAR_BITMASK;

    bool anyError =
    (
        _last_reading.errors.dataInvalid  ||
        _last_reading.errors.noData       ||
        _orbis_errors.counter_error       ||
        _orbis_errors.speed_high          ||
        _orbis_errors.temp_out_of_range   ||
        _orbis_errors.dist_far            ||
        _orbis_errors.dist_near
    );
